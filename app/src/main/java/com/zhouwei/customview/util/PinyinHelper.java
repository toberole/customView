...
public class PinyinHelper {
    ...
    public static PinyinHelper getInstance() {
        synchronized (PinyinHelper.class) {
            ...
            Log.w(TAG, "There is no Chinese collator, PinyinHelper is disabled");
            sInstance = new PinyinHelper(false);
            return sInstance;
        }
    }
    ...
    public static String hanZiToPinyin(String input) {
        ArrayList<Token> tokens = PinyinHelper.getInstance().get(input);
    ...
    public static String[] hanZiToPinyinArray(String input) {
        String res[] = new String[input.length()];
        for (int i = 0; i < input.length(); i++) {
            String tt = input.charAt(i) + "";
            ArrayList<Token> tokens = PinyinHelper.getInstance().get(tt);
            StringBuilder sb = new StringBuilder();
            if (tokens != null && tokens.size() > 0) {
                for (Token token : tokens) {
                    if (Token.PINYIN == token.type) {
